...
df = spark.read.csv(
    r'c:\Users\yamun\Downloads\HealthCare\NH_ProviderInfo_Oct2024.csv', header=True, inferSchema=True)

# df = pd.read_csv(
#     r'c:\Users\yamun\Downloads\HealthCare\NH_ProviderInfo_Oct2024.csv', encoding='cp1252')

# print(df.dtypes)

# for column_name, data_type in enumerate(df.dtypes):
#     print(column_name, data_type)

df = df.select(
    "Affiliated Entity ID",
    F.col("Affiliated Entity ID").isNull().alias("is_null")
).show()

df = df.filter(F.col("county_code").isNull()).count()
...
print(df['CMS Certification Number (CCN)'].dtype)
